...
class BestBuyScanner:
    def __init__(self):
        self.sa = SendAlert()
    def getJSONPageCount(self,key):
        url=f'https://api.bestbuy.com/v1/products(search=rtx&search=3070&search=graphics&search=card)?format=json&show=sku&apiKey={key}'
        r=requests.get(url)
        self.pageCount= r.json()["totalPages"]

    def scanBBStock(self, key, email, password,notifyContact, port):
